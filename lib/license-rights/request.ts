...
export const DEFAULT_REQUEST_MESSAGE = `I purchased the Full Turnkey Reseller & License Rights Edition and would like the team to activate it on my ${support.productName} account.
...
export type SubmitLicenseRightsResult = { ok: true; viaMailto: boolean } | { ok: false; error: string }

function pendingStorageKey(userId: string): string {
  return `fmc_license_rights_request_${userId || "anonymous"}`
}
