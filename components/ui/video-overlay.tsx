...
/** Free Money Cash bonus / scale training offer (same destination as earnings banner). */
const BONUS_TRAINING_URL = "https://freedomescapexcelerator.com/2k-per-day"
  ...
  try {
    ...
    if (u.hostname === "player.vimeo.com" || /(^|\.)vimeo\.com$/.test(u.hostname)) {
      if (u.hostname !== "player.vimeo.com") {
        ...
        const id = parts[0]
        const privacyHash = parts[1] && !/^\d+$/.test(parts[1]) ? parts[1] : null
        if (!id) return null
        u = new URL(`https://player.vimeo.com/video/${id}`)
        if (privacyHash && !u.searchParams.has("h")) u.searchParams.set("h", privacyHash)
      }
...
/**
 * Full-screen / modal video player with a promo bar under the player
 * (ProfitLoop overflow pattern).
 */
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: "0 0 0 1px rgba(207,161,59,0.12) inset, 0 24px 80px rgba(0,0,0,0.7)",
        }}
      >
        <div
            ...
            <div className="flex flex-1 items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#22d38b]">
                  Member Bonus Unlocked
                </p>
                <p className="mt-1 text-base font-semibold leading-snug text-white">
                  Free training that shows how to scale to{" "}
                  <span className="font-extrabold text-[#22d38b]">$1,000–$5,000/day</span>
                </p>
                <p className="mt-0.5 text-xs font-medium text-emerald-100/55">
                  Included for Free Money Cash members
                </p>
              </div>
            </div>
            <a
              href={BONUS_TRAINING_URL}
              target="_blank"
              rel="noopener noreferrer"
